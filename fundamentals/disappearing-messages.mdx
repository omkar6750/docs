...
description: "Allow users to send messages that disappear after a set interval or time."
---

Disappearing Messages allows users to send messages that automatically delete after a specified time interval. This works for both one-on-one and group conversations, also known as exploding messages on some platforms.
...
<Info>Disappearing Messages works with CometChat SDKs via the `callExtension` method. UI Kit support may vary — check your platform's UI Kit documentation for details.</Info>
