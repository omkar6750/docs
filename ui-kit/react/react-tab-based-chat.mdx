...
<CometChatConversations onItemClick={(conversation) => handleSelect(conversation)} />  // Chats tab
<CometChatCallLogs onItemClick={(call) => handleSelect(call)} />                        // Calls tab
<CometChatUsers onItemClick={(user) => handleSelect(user)} />                           // Users tab
<CometChatGroups onItemClick={(group) => handleSelect(group)} />                        // Groups tab
```
...
<CardGroup cols={2}>
  ...
  <Card title="Call Logs" icon="phone" href="/ui-kit/react/call-logs">
