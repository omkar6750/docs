...
description: "Step-by-step migration guide from CometChat React UI Kit v5 to v6 covering localization, language codes, translations, and date formatting changes"
---
...
- **Init:** `init(language, resources)` → `init(settings: LocalizationSettings)`
- **Language codes:** `en` → `en-US` (English expanded to regional variants)
- **Translations:** Use `addTranslation()` method or `translationsForLanguage` in init
- **Date formatting:** New `CalendarObject` for customizable date/time display
- **Missing keys:** New `missingKeyHandler` callback
- **Property changes:** See [Property Changes](/ui-kit/react/property-changes)
</Info>
...
<CardGroup cols={2}>
  <Card title="Property Changes" icon="list-check" href="/ui-kit/react/property-changes">
    Detailed list of new, renamed, and removed properties
  </Card>
  ...
  <Card title="Getting Started" icon="rocket" href="/ui-kit/react/react-js-integration">
    Set up a fresh v6 project from scratch
  </Card>
