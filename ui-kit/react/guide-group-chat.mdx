...
description: "Learn how to implement comprehensive group management functionality including creation, joining, member management, and administrative controls in your React chat app."
---
...
CometChat.createGroup(group).then((createdGroup) => {
  CometChatGroupEvents.ccGroupCreated.next(createdGroup);
});

// Join a group
CometChat.joinGroup(guid, groupType, password).then((joinedGroup) => {
  CometChatGroupEvents.ccGroupMemberJoined.next({ joinedGroup, joinedUser });
});
...
<CardGroup cols={2}>
  <Card title="Groups Component" icon="users-rectangle" href="/ui-kit/react/groups">
