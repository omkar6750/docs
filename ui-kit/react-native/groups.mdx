---
title: "Groups"
description: "Display and manage group lists with the CometChatGroups component for React Native."
---
...
// With common props
<CometChatGroups
  onItemPress={(group) => handleGroupSelect(group)}
  selectionMode="none"
  hideSearch={false}
/>
```
...
const onLongPressHandler = (group: CometChat.Group) => {
  console.log("Long pressed:", group);
};
...
const onLongPressHandler = (group) => {
  console.log("Long pressed:", group);
};
...
const onSelectionHandler = (selection: Array<CometChat.Group>) => {
  console.log("Selected groups:", selection);
};
...
CometChatUIEventHandler.addGroupListener("GROUP_LISTENER_ID", {
  ccGroupLeft: ({ leftGroup }: { leftGroup: CometChat.Group }) => {
    console.log("Group left:", leftGroup);
  },
});
