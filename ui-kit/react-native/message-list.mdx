---
title: "Message List"
description: "Display and manage message lists with the CometChatMessageList component for React Native."
---
...
<CometChatMessageList
  ...
  onThreadRepliesPress={(message, bubbleView) => handleThreadPress(message)}
/>
```
</Info>
...
const threadRepliesCallback = (msg, messageBubbleView) => {
  //handle thread
};
...
const onReactionLongPress = (reaction, messageObject) => {};
...
const onReactionPress = (reaction, messageObject) => {};
...
const onReactionListItemPress = (reaction, messageObject) => {};
...
CometChatUIEventHandler.addUIListener("UI_LISTENER_ID", {
  openChat: ({ user, group }: { user?: CometChat.User; group?: CometChat.Group }) => {
    //code
  },
});
...
CometChatUIEventHandler.addMessageListener("MESSAGE_LISTENER_ID", {
  ccMessageDelivered: ({ message }: { message: CometChat.BaseMessage }) => {
    //code
  },
});
...
`CometChatMessageTemplate` is a pre-defined structure for creating message views that can be used as a starting point or blueprint for creating message views often known as message bubbles. For more information on styling message bubbles, refer to [Message Bubble Styling](/ui-kit/react-native/message-bubble-styling).
...
function App() {
  ...
  const getTemplates = () => {
    let templates = ChatConfigurator.getDataSource().getAllMessageTemplates(theme);
    templates.map((data) => {
      if (data.type == "text") {
        data.ContentView = (messageObject, alignment) => {
          return (
            <Text style={{ backgroundColor: "#fff5fd", padding: 10 }}>
...
function App() {
  ...
  const customDateSeparatorPattern = (date) => {
    const timeStampInSeconds = new Date(date * 1000);
...
function App(): React.JSX.Element {
  ...
  const generateDateString = (message: CometChat.BaseMessage): string => {
    const days = [
      "Sunday",
      "Monday",
      "Tuesday",
  ...
  return (
    <>
      {chatUser && (
        <CometChatMessageList user={chatUser} datePattern={generateDateString} />
      )}
    </>
  );
}
...
function App() {
  ...
  const generateDateString = (message) => {
    const days = [
      "Sunday",
      "Monday",
      "Tuesday",
...
const getAIGreetingView = (): JSX.Element => {
  return (
...
const aiSuggestedMessages: string[] = [
  "How can I help you today?",
...
const handleSuggestedMessageClick = (suggestion) => {
  console.log("Suggested message clicked:", suggestion);
  // Handle the suggested message click
};
...
return <CometChatMessageList user={user} hideSuggestedMessages={true} />;
```
</Tab>
<Tab title="JavaScript">
```jsx
...
const getAIIntroView = (): JSX.Element => {
  return (
    <View style={{ padding: 16 }}>
      <Text style={{ fontSize: 16, textAlign: "center" }}>
        Welcome! I'm here to assist you with any questions or tasks you might
        have.
      </Text>
    </View>
  );
...
return (
  <CometChatMessageList user={user} emptyChatIntroMessageView={getAIIntroView()} />
);
```
</Tab>
<Tab title="JavaScript">
...
const getAIImageView = (): JSX.Element => {
  return (
...
return <CometChatMessageList group={group} textFormatters={getTextFomatters()} />;
```
</Tab>
<Tab title="JavaScript">
```jsx
...
const getMessageListHeaderView = ({ user, group, id }) => {
  //return jsx;
};
...
const getMessageListFooterView = ({ user, group, id }) => {
  //return jsx;
};
...
const CustomNewMessageIndicator = (): JSX.Element => {
  return (
    <View
      style={{
...
<CardGroup cols={2}>
  <Card title="Message Composer" icon="pen-to-square" href="/ui-kit/react-native/message-composer">
    Compose and send messages in a conversation
  </Card>
