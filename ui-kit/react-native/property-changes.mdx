...
description: "Complete reference of property changes between React Native UI Kit v4 and v5, including new, renamed, and removed properties for all components."
---

<Info>
**Quick Reference for AI Agents & Developers**

Key property changes in v5:
- **Naming conventions**: `*StateView` → `*View` (e.g., `EmptyStateView` → `EmptyView`)
- **Trailing views**: `TailView` → `TrailingView` across all components
- **Item views**: `ListItemView` → `ItemView` for custom list rendering
- **Style props**: Component-specific styles (e.g., `conversationsStyle`) → unified `style` prop
- **User presence**: `disableUsersPresence` → `usersStatusVisibility` (inverted logic)
- **Receipts**: `hideReceipt`/`disableReceipt` → `receiptsVisibility`
- **Back button**: `showBackButton`/`hideBackIcon` → `hideBackButton` (check inverted logic)

Common patterns:
```tsx
// V4 → V5 style migration
// Before (v4)
<CometChatConversations conversationsStyle={myStyle} />

// After (v5)
<CometChatConversations style={myStyle} />

// V4 → V5 view migration
// Before (v4)
<CometChatConversations EmptyStateView={MyEmpty} TailView={MyTail} />

// After (v5)
<CometChatConversations EmptyView={MyEmpty} TrailingView={MyTail} />
```
</Info>
...
<CardGroup cols={2}>
  <Card title="Upgrading from V4" icon="arrow-up" href="/ui-kit/react-native/upgrading-from-v4">
    Step-by-step migration guide from v4 to v5
  </Card>
