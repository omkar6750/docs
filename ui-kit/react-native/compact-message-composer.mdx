...
```json
{
  ...
  "props": {
    ...
    "richText": {
      ...
      "showToolbarToggle": { "type": "boolean", "default": true },
...
Fires when the rich text formatting toolbar visibility changes.
...
function CompactComposerWithToolbarTracking() {
  return (
    <CometChatCompactMessageComposer
      user={chatUser}
      enableRichTextEditor={true}
      onToolbarVisibilityChange={(visible: boolean) => {
...
| `enableRichTextEditor` | `hideRichTextFormattingOptions` | `showToolbarToggle` | Result |
|------------------------|-------------------------------|---------------------|--------|
| `false` | - | - | Plain text, no formatting UI |
| `true` | `true` | `false` | Toolbar always visible above input |
| `true` | `false` | `false` | No toolbar UI, only text selection menu items work |
...
| `showToolbarToggle` | Show Aa toggle button for toolbar | `showToolbarToggle={true}` |
