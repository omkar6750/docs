...
description: "Guide to migrating your React Native app from CometChat UI Kit v4 to v5, covering component changes, theming updates, and property modifications."
---

<Info>
**Quick Reference for AI Agents & Developers**

Key migration steps from v4 to v5:
- **Component Changes**: Composite components like `CometChatConversationsWithMessages` are replaced with modular components (`CometChatConversations`, `CometChatMessageList`, `CometChatMessageComposer`, etc.)
- **Theming**: Replace `CometChatTheme` class with `CometChatThemeProvider` component and direct theme object configuration
- **Styling**: Use theme provider for global styles and component props for local customization
- **Properties**: Style props simplified, configuration props reduced, custom view props standardized (`itemView`, `leadingView`, `trailingView`, `subtitleView`, `titleView`)
...
<CardGroup cols={2}>
  <Card title="Property Changes" icon="list-check" href="/ui-kit/react-native/property-changes">
    Complete reference of added, renamed, and removed properties in v5
  </Card>
  <Card title="Getting Started" icon="rocket" href="/ui-kit/react-native/getting-started">
    Step-by-step guide to building your first chat with v5 UI Kit
  </Card>
</CardGroup>