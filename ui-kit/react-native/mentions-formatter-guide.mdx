...
description: "Learn how to use CometChatMentionsFormatter to format and handle @mentions in your React Native chat application."
---
...
```tsx
import { CometChatMentionsFormatter, CometChatTextFormatter } from "@cometchat/chat-uikit-react-native";

// Initialize mentions formatter
const mentionsFormatter = new CometChatMentionsFormatter();

// Add to text formatters array
const textFormatters: Array<CometChatTextFormatter> = [mentionsFormatter];

// Use with Message List or Message Composer
<CometChatMessageList textFormatters={textFormatters} />
<CometChatMessageComposer textFormatters={textFormatters} />
```

This guide covers: [Mentions Formatter Setup](#usage) | [Click Handling](#setOnMentionClick) | [Styling](#mentionTextStyle)
</Info>
...
const mentionsFormatter: CometChatMentionsFormatter = new CometChatMentionsFormatter();
const textFormatters: Array<CometChatTextFormatter> = [mentionsFormatter];
...
```jsx
import { CometChatMentionsFormatter } from "@cometchat/chat-uikit-react-native";

// Initialize CometChatMentionsFormatter
const mentionsFormatter = new CometChatMentionsFormatter();
const textFormatters = [mentionsFormatter];
...
function App() {
    ...
    mentionsFormatter.setOnMentionClick((message, uid) => {
       const mentionedUsers = message.getMentionedUsers();
       let user;
       for(let i = 0; i < mentionedUsers.length; i++) {
           if(uid == mentionedUsers[i].getUid()) {
...
<CardGroup cols={2}>
  <Card title="Shortcut Formatter Guide" icon="keyboard" href="/ui-kit/react-native/shortcut-formatter-guide">
    Create custom keyboard shortcuts for text formatting
  </Card>
