...
| `resizeToAvoidBottomInset` | `bool` | Must match the parent `Scaffold`'s value. When `false`, the composer manages keyboard height, safe area padding, and sticker panel transitions internally. Default: `false`. |
