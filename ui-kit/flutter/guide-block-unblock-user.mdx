...
description: "Enable users to block and unblock other users for privacy and spam control"
---
...
- `CometChatUIKit.blockUsers()` → SDK method to block users
- `CometChatUIKit.unblockUsers()` → SDK method to unblock users
- `CometChatUserInfo` → User profile screen
- `User.blockedByMe` → Block status property (SDK)
...
<CardGroup cols={2}>
  ...
  <Card title="Group Management" icon="user-group" href="/ui-kit/flutter/guide-group-chat">
