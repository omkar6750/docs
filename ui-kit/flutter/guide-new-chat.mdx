...
description: "Enable users to start one-on-one or group chats with a searchable contact list"
---
...
// Add avatar menu with "Create Conversation" option
PopupMenuButton(
...
CometChatUsers(
  onItemTap: (context, user) => PageManager.navigateToMessages(context: context, user: user),
);
```

**Key Components:**
- `CometChatAvatar` → Avatar display widget
- `CometChatContacts` → Contact selection screen
- `CometChatUsers` → [Docs](/ui-kit/flutter/users)
- `CometChatGroups` → [Docs](/ui-kit/flutter/groups)
- `CometChatContactsController` → Tab switching controller (sample app)

**Sample Implementation:** [contacts/cometchat_contacts_controller.dart](https://github.com/cometchat/cometchat-uikit-flutter/blob/v5/sample_app/lib/contacts/cometchat_contacts_controller.dart)
</Info>
