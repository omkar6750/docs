...
<Note>
The image URL must be accessible from the user's browser. For better performance, consider hosting images on a CDN. If the image is loaded from a different origin, ensure the appropriate CORS headers (e.g., `Access-Control-Allow-Origin`) are set on the server hosting the image.
</Note>
