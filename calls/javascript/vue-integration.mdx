...
This guide walks you through integrating the CometChat Calls SDK into a Vue.js application. By the end, you'll have a working video call implementation with proper lifecycle handling.
...
const AUTH_KEY = "YOUR_AUTH_KEY";     // Replace with your Auth Key
...
export function useCometChatCalls() {
  ...
  async function initAndLogin(uid) {
    ...
    try {
      ...
      if (!loggedInUser) {
        loggedInUser = await CometChatCalls.login(uid, AUTH_KEY);
      }
