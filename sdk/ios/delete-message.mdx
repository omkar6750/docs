...
Use `onMessageDeleted` in `CometChatMessageDelegate` to receive real-time delete events.
...
When fetching message history, deleted messages have `deletedAt` and `deletedBy` fields set. Additionally, an `ActionMessage` is added to history indicating the deletion.

The `ActionMessage` object contains:
- `action` — `deleted`
