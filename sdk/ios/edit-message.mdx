...
Use `onMessageEdited` in `CometChatMessageDelegate` to receive real-time edit events.
...
When fetching message history, edited messages have `editedAt` and `editedBy` fields set. Additionally, an `ActionMessage` is added to history indicating the edit.
