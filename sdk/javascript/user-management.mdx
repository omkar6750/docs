...
However, if you wish to create users on the fly, you can use the `createUser()` method. This method takes a [`User`](/sdk/reference/entities#user) object and the `Auth Key` as input parameters and returns the created `User` object if the request is successful.
...
Please make sure the [`User`](/sdk/reference/entities#user) object provided to the `updateUser()` method has the `UID` of the user to be updated set.
