...
CometChat supports multiple message types to cover all your chat needs. For a deeper understanding of how messages are structured, see [Message Structure & Hierarchy](/sdk/javascript/message-structure-and-hierarchy).
...
<Tab title="Async/Await">
```javascript
async function sendAndLogMessage(textMessage) {
  try {
    const message = await CometChat.sendMessage(textMessage);
    console.log("Message ID:", message.getId());
    console.log("Sent at:", message.getSentAt());
