...
Once initialization is successful, you will need to create a user. To create users on the fly, you can use the `createUser()` method. This method takes a [`User`](/sdk/reference/entities#user) object and the `Auth Key` as input parameters and returns the created `User` object if the request is successful.
...
The `login()` method returns the [`User`](/sdk/reference/entities#user) object on `Promise` resolved containing all the information of the logged-in user.
