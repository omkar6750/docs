...
Before you begin, we strongly recommend you read the [calling setup guide](/sdk/android/v5/calling-setup).

<Note>

If you want to implement a complete calling experience with ringing functionality (incoming/outgoing call UI), follow the [Ringing](/sdk/android/v5/default-calling) guide first. Once the call is accepted, return here to start the call session.
...
When using the [Ringing](/sdk/android/v5/default-calling) flow, you must coordinate between the CometChat Chat SDK and the Calls SDK to properly terminate the call and notify all participants.
