...
Use `editMessage()` with a [`TextMessage`](/sdk/javascript/reference/messages#textmessage) or [`CustomMessage`](/sdk/javascript/reference/messages#custommessage) object. Set the message ID using the `id` property.
...
The `editMessage()` method returns a [`BaseMessage`](/sdk/javascript/reference/messages#basemessage) object (or a subclass like [`TextMessage`](/sdk/javascript/reference/messages#textmessage)).
...
The `onMessageEdited` callback receives a [`BaseMessage`](/sdk/javascript/reference/messages#basemessage) object with the `editedAt` and `editedBy` fields set.
...
When fetching message history, edited messages have `editedAt` and `editedBy` fields set. Additionally, an [`Action`](/sdk/javascript/reference/messages#action) message is created when a message is edited.

The [`Action`](/sdk/javascript/reference/messages#action) object contains:
- `action` — `edited`
