...
- `authKey` enables guest logins; never expose your REST API key.
- Set `defaultChatID` to the AI agent's UID so the conversation opens immediately.
  </Tab>
...
- Pass the same `uid` you use in your app so messages map back to the correct person.
- The optional `user` object lets you sync profile metadata shown next to the agent thread.
  </Tab>
...
- Generate the auth token on your backend via the [Auth Token API](/rest-api/auth-tokens/create) and send it to the browser over HTTPS.
- This mode keeps API keys server-side while still opening the agent chat automatically.
  </Tab>
</Tabs>
