...
<script>
  ...
  const COMETCHAT_LAUNCH_OPTIONS = {
    ...
    chatType: "user",
    defaultChatID: "<DEFAULT_AGENT_OR_USER_ID>",  // Agent/User ID to open chat by default
...
> Replace the placeholder values (`<YOUR_APP_ID>`, etc.) with the credentials, user UID, default chat ID, and variant ID from your deployment.

### Choose an Authentication Flow

Use the same embed script plus the lightweight `CometChatAuth` helper to create/login users before the widget launches. Pick the mode that matches how people sign in to your product.

<Callout>
Replace <code>https://cdn.example.com/cometchat/cometchat-auth.js</code> with the `cometchat-auth.js` URL shared by your CometChat account team.
</Callout>

<Tabs>
  <Tab title="Anonymous (Guest Mode)">
Let anyone chat with the agent without creating an account—great for demos or marketing sites.
...
<script type="module">
  ...
  const COMETCHAT_WIDGET_CONFIG = {
    ...
    chatType: "user",
    defaultChatID: "<DEFAULT_AGENT_OR_USER_ID>"
  };
...
<ul>
  <li><code>authKey</code> enables guest logins; never expose your REST API key.</li>
  <li>Set <code>defaultChatID</code> to the AI agent UID so the conversation opens immediately.</li>
</ul>
  </Tab>
  <Tab title="Create + Log In User (UID Mode)">
Reuse the IDs your product already issues (email, handle, etc.). CometChat creates the user on first visit and reuses it afterward.
...
<ul>
  <li>Pass the same <code>uid</code> you use in your app so messages map back to the correct person.</li>
  <li>The optional <code>user</code> object keeps the widget avatar/name in sync with your profile data.</li>
</ul>
  </Tab>
  <Tab title="Backend Auth Token (Secure Mode)">
If you already authenticate users on your server, create a CometChat auth token after they log in and hand it to the page.
...
<ul>
  <li>Generate the auth token on your backend via the <a href="/rest-api/auth-tokens/create">Auth Token API</a> and send it to the browser over HTTPS.</li>
  <li>This mode keeps API keys server-side while still opening the agent chat automatically.</li>
</ul>
  </Tab>
</Tabs>
