...
- Parse NDJSON events; render `text_delta`, surface `tool_call_*` and `tool_result` if you want UI actions, stop on `text_end`/`done`.
- Replace the mock launch data with Product Hunt GraphQL/Algolia or your own source while keeping the tool signatures (`list_top_posts`, `search_launches`) intact.
- Add more tools (e.g., “draft_launch_post”, “schedule_announcement”) before compiling the graph.
