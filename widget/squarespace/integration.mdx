...
Add the CometChat Widget with a small snippet. It drops in like any other embed—copy the code, paste it into Squarespace, then pick how people sign in.
...
### Squarespace placement (applies to all login options)

- In **Website → Pages → Custom Code → Header**, add:

```html
<script defer src="https://cdn.jsdelivr.net/npm/@cometchat/chat-embed@1.x.x/dist/main.js"></script>
```

- In **Website → Pages → Custom Code → Footer** (site-wide) or a page **Code Block** (page-specific), paste the matching snippet below. Keep the mount `<div>` and the `script type="module"` together in the same place.
...
### Paste this in Footer or a Code Block
