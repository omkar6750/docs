...
<Info>
  **Quick Reference for AI Agents & Developers** - **What this page covers**:
  Three ways to integrate the CometChat Widget into a static HTML page —
  anonymous/guest mode, UID-based login, and backend auth token login. -
  **Prerequisites**: A CometChat account with App ID, Region, and Auth Key from
  the [Dashboard](https://app.cometchat.com/). - **Key action**: Pick a sign-in
  method below, copy the HTML snippet, and paste it into your page.
</Info>
...
- For **production environments**, use an **Auth Token** (see [Backend-Created User](#3-backend-created-user-auth-token-login)) instead of an **Auth Key** to enhance security and prevent unauthorized access.
  </Note>
...
<CardGroup rows={3} className="mt-6">
  <Card title="Anonymous Chat (Guest Mode)" href="#1-anonymous-chat-guest-mode">
    - Let anyone chat anonymously without signing up or logging in. - Perfect
    for marketing pages, help centers, or demo sites.
  </Card>
  <Card
    title="Create + Log In User On The Fly"
    href="#2-create-+-log-in-user-on-the-fly"
  >
    - Use your existing user IDs (email, username etc.) to create and log in
    users automatically. - No backend needed—CometChat creates users the first
    time they visit.
  </Card>
  <Card
    title="Backend-Created User (Auth Token Login)"
    href="#3-backend-created-user-auth-token-login"
  >
    - Create your users via server and login them using secure auth token on
    frontend. - Ideal for sites with existing login systems and backends.
  </Card>
</CardGroup>
...
<CardGroup cols={2}>
  <Card title="Advanced Setup" href="/widget/html/advanced">
    Explore advanced JavaScript controls — open specific chats, listen for
    events, manage users, and more.
  </Card>
  ...
  <Card title="Widget Overview" href="/widget/html/overview">
    Go back to the Widget Builder overview for a high-level walkthrough.
  </Card>
</CardGroup>
