...
- Manifest: point `android:name` on `<application>` to your `MyApplication` class; ensure service/receiver class names match your package; replace app name/icons in `AndroidManifest.xml` / `res`.
- In [`AppConstants.kt`](https://github.com/cometchat/cometchat-uikit-android/blob/v5/sample-app-kotlin%2Bpush-notification/src/main/java/com/cometchat/sampleapp/kotlin/fcm/utils/AppConstants.kt), keep channel IDs, group keys, notification IDs, and intent extra keys. If you change package names or channel names, update them across manifest and notification builders.

**Splash/entry deep link handler** (update target activity as needed):
...
**SplashViewModel (headless init + login check)**
...
```kotlin
object Repository {
    fun registerFCMToken(listener: CometChat.CallbackListener<String>) { /* ...as above... */ }
    fun unregisterFCMToken(listener: CometChat.CallbackListener<String>) { /* ...as above... */ }
