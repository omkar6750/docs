...
private fun handleDeepLinking() {
    ...
    if (type == AppConstants.FCMConstants.NOTIFICATION_TYPE_MESSAGE) {
        ...
        val chatFragment = getFragment(R.id.nav_chats)
        chatFragment.arguments = Bundle().apply {
            putString(AppConstants.FCMConstants.NOTIFICATION_TYPE, type)
            putString(AppConstants.FCMConstants.NOTIFICATION_PAYLOAD, payload)
        }
