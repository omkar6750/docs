...
and generate a new private key and upload the JSON file when we create the FCM provider on CometChat dashboard.
...
Minimal pieces from `Repository` that are relevant to push tokens (adapt package/imports to your namespace):
            ...
            if (task.isSuccessful) {
                ...
                CometChatNotifications.registerPushToken(
                    ...
                    object : CometChat.CallbackListener<String?>() {
                        override fun onSuccess(s: String?) { listener.onSuccess(s) }
                        override fun onError(e: CometChatException) { listener.onError(e) }
                    }
                )
            } else {
...
Make sure you actually set these at runtime:

- `isAppInForeground` and `currentActivity` are set inside the lifecycle callbacks shown above.
- `currentOpenChatId`: set when a chat screen opens and clear on exit so message notifications are suppressed only for the active chat.
- `tempCall` via `setTempCall(...)`: set when an incoming call arrives (before showing the banner) and clear on dismiss/end; `getTempCall()` is read when resuming to re-show the banner.
