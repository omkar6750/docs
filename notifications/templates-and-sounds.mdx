...
Applies to text messages (`category: "message"`, `type: "text"`) where the message contains `@all` in a group conversation. @all mentions are strictly a group concept — they do not apply to one-on-one conversations, so there are no one-on-one title or body fields.
