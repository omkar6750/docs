...
##### Controlling notifications for custom messages

Custom category messages require an explicit opt-in to trigger notifications. The `sendNotification` flag is a boolean field on the message's data payload that controls whether CometChat processes the message for push, email, and SMS notifications.

| `sendNotification` value | Behavior |
| --- | --- |
| `true` | Notifications are processed normally (push, email, SMS) |
| `false` or not set | The message is silently skipped — no notifications are sent |

<Warning>
By default, custom messages without `sendNotification: true` explicitly set will not trigger any notifications. The sender must opt in by including this flag in the message payload.
</Warning>

This applies uniformly across all notification channels — push, email, and SMS. The check runs before any template resolution or delivery logic, so a skipped message produces no notification activity at all.

**When to use this:**
- Set `sendNotification: true` for custom messages that represent user-facing activity (e.g., polls, payments, shared locations) where the recipient should be alerted.
- Omit the flag (or set it to `false`) for background or system-level custom messages that should not interrupt the recipient (e.g., typing indicators sent as custom messages, metadata syncs, or silent state updates).
