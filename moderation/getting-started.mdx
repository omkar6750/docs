...
<Steps>
  ...
  <Step title="Integrate with SDK (Custom UI only)">
...
For server-side integrations or advanced use cases, you can also use the REST API directly:

| Endpoint | Purpose |
|----------|---------|
| [Send Message](/rest-api/moderation) | Submit content for moderation |
| [List Messages](/rest-api/moderation) | Retrieve moderated messages with filters |
| [Approve Message](/rest-api/moderation) | Manually approve a blocked message |

<Note>
For most use cases, the SDK integration above is recommended as it handles moderation automatically.
</Note>
...
- **REST API**: Available for server-side or advanced integrations
